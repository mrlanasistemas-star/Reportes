...
<template>
    <div class="rounded-2xl border bg-white shadow-sm">
        <div class="flex items-center justify-between border-b p-4">
            <div>
                <h3 class="text-xs font-black uppercase tracking-wider text-slate-500">Comparativo de métricas</h3>
                <p class="mt-0.5 text-[11px] text-slate-400">Todas las métricas disponibles para este alcance — nunca se ocultan de forma permanente.</p>
            </div>
            <button type="button" @click="showAll = !showAll"
                    class="h-8 shrink-0 rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-600 transition hover:bg-slate-50">
        ...
        <div class="max-h-[520px] overflow-auto">
            <table class="w-full text-sm">
                ...
                <tbody>
                    <tr v-for="(row, i) in visibleRows" :key="row.label" :class="i % 2 === 1 ? 'bg-slate-50/70' : ''" class="border-b border-slate-100">
                        <td class="px-4 py-2.5 font-semibold text-slate-800">{{ row.label }}</td>
                        <td class="px-4 py-2.5 text-right text-slate-700">{{ fmtValue(row.curr, row.fmt) }}</td>
                        <td class="px-4 py-2.5 text-right text-slate-500">{{ fmtValue(row.prev, row.fmt) }}</td>
                    ...
                    <tr v-if="visibleRows.length === 0">
                        <td colspan="5" class="px-4 py-8 text-center text-sm text-slate-400">Sin métricas para este alcance.</td>
                    </tr>
                </tbody>
            </table>
        </div>
